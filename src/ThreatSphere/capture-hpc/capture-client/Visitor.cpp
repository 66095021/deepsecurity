...
{	
	...
	connect_onCMDEvent(boost::bind(&Visitor::onCMDEvent, this, _1, _2));
...
void Visitor::onCMDEvent(const std::string& application, const std::string& parameter)
{
	char file[1024]={0};
	sprintf(file, "%s%s",TEMP_FILE_FOLDER, parameter.c_str());
	std::wstring wsfile = StringHelper::multiByteStringToWideString(file, strlen(file)+1);
	std::wstring wsapp = StringHelper::multiByteStringToWideString(application.c_str(), application.size()+1);

	VisitCmdEvent* cmdEvent = new VisitCmdEvent();
	cmdEvent->setProgram(wsapp);
	cmdEvent->setParameter(wsfile);
	
	VisitQueue.push(cmdEvent);
...
Visitor::loadClientPlugins()
{
	...
	if (hFind != INVALID_HANDLE_VALUE) 
	{
		...
		//	HMODULE hPlugin = LoadLibrary(pluginDir.c_str());
			HMODULE hPlugin = LoadLibrary(_T("plugins\\Application_ClientConfigManager.dll"));
...
Visitor::run()
{
	try {
		...
		//	WaitForSingleObject(hQueueNotEmpty, INFINITE);
			...
			VisitCmdEvent* visitEvent = VisitQueue.front();
			VisitQueue.pop();
			visiting = true;
			...
			if(applicationPlugin == NULL)
			{
				LOG(ERR, "\tnot supported program: %ls", visitEvent->getProgram());
			}
		//	_ASSERT(applicationPlugin != NULL);
			if(applicationPlugin)
			{
				...
				try
				{
					applicationPlugin->visit(visitEvent);
				} 
				catch (...)
				{

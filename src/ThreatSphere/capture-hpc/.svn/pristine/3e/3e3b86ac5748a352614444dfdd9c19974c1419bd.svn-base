...
BOOLEAN CheckEventHandler(EventHandler* event_handler)
{
	if(event_handler == NULL || event_handler->node == NULL  ||
		event_handler->original_handler.EventHandler == NULL ||
		event_handler->original_handler.EventContext == NULL)
		return FALSE;
	else
		return TRUE;
}
								...
								)
{
	...
	if(!CheckEventHandler(event_handler))
		return STATUS_SUCCESS; //or STATUS_DATA_NOT_ACCEPTED ??

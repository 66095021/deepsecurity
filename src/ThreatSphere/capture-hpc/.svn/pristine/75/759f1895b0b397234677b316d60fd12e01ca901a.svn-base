...
VOID UnloadDriver( IN PDRIVER_OBJECT driver_object )
{
	...
	ConnectionManager* connection_manager = NULL;
	PDEVICE_OBJECT	obDevices[2] = { 0 };
	PDEVICE_OBJECT	obDevice = NULL;
	LONG			idex;
	LARGE_INTEGER	liWait;
 	liWait.QuadPart = -10;

	obDevices[0] = driver_object->DeviceObject;
	obDevices[1] = obDevices[0]->NextDevice;

	for (idex = 0; idex <=1; ++idex)
	{


		connection_manager = (ConnectionManager*)obDevices[idex]->DeviceExtension;
		...
		while (InterlockedCompareExchange(&connection_manager->nNumberOfIrps, 0, 0) != 0)
		{
			DbgPrint("CaptureConnectionMonitor: Unloaded wait irps: %d\n",connection_manager->nNumberOfIrps);
 			KeDelayExecutionThread(KernelMode, FALSE, &liWait);
		}
		...
		IoDeleteDevice(obDevices[idex]);

		DbgPrint("CaptureConnectionMonitor: Unloaded %ls\n", device_name.Buffer);
	}
}
...
//			DbgPrint("TDI IRP_MJ_DEVICE_CONTROL\t\t%08x\n", pirp_stack->FileObject);
	}
												...
												CompletionRoutineContext* context)
{
	...
	InterlockedIncrement(&connection_manager->nNumberOfIrps);
...
NTSTATUS TdiNewIrpCompletionHandler(PDEVICE_OBJECT device_object, PIRP irp, PVOID context)
{
	...
	ConnectionManager* ConnMgr = (ConnectionManager*)device_object->DeviceExtension;
	...
	InterlockedDecrement(&ConnMgr->nNumberOfIrps);

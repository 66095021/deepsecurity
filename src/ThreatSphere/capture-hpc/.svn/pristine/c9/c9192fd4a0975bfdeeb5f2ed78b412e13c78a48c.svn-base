...
VOID ThreadCallback(IN HANDLE  hProcessId, IN HANDLE  hThreadId, IN BOOLEAN  bCreate);
	...
	)
{
	...
	/* Register thread callback function */
	ntStatus = PsSetCreateThreadNotifyRoutine(ThreadCallback);
	if(!NT_SUCCESS(ntStatus))
	{
		DbgPrint("CaptureProcessMonitor: ERROR PsSetCreateThreadNotifyRoutine - %08x\n", ntStatus); 
		return ntStatus;
	}
...
VOID ResetProcessEvent(PROCESS_EVENT* pProcessEvent)
{
//	pProcessEvent->time= 0;
	pProcessEvent->hParentProcessId = 0;
	pProcessEvent->hOriginProcessId = 0;
	pProcessEvent->hProcessId = 0;
	pProcessEvent->hThreadId = 0;

	pProcessEvent->bCreated = 0;

	pProcessEvent->processPathLength = 0;
	pProcessEvent->parentProcessPathLength = 0;
	pProcessEvent->originProcessPathLength = 0;

	RtlZeroMemory(pProcessEvent->processPath, sizeof(pProcessEvent->processPath));
	RtlZeroMemory(pProcessEvent->parentProcessPath, sizeof(pProcessEvent->parentProcessPath));
	RtlZeroMemory(pProcessEvent->originProcessPath, sizeof(pProcessEvent->originProcessPath));	
}
	...
	)
{
	...
	UNICODE_STRING parentProcImagePath;
	...
	ResetProcessEvent(&(processEventPacket->processEvent));
	...
	parentProcImagePath.Length = 0;
	parentProcImagePath.MaximumLength = 0;
	...
	status = GetProcessImageName(hParentId, &parentProcImagePath);
	if (status == STATUS_BUFFER_OVERFLOW)
	{
		parentProcImagePath.Buffer = ExAllocatePoolWithTag(NonPagedPool, parentProcImagePath.MaximumLength, PROCESS_POOL_TAG);
		if (parentProcImagePath.Buffer != NULL)
		{
			status = GetProcessImageName(hParentId, &parentProcImagePath);
			if (NT_SUCCESS(status))
			{
				//DbgPrint("CaptureProcessMonitor: %i %i=>%i:%wZ\n", bCreate, hParentId, hProcessId, &processImagePath);
				RtlStringCbCopyUnicodeString(processEventPacket->processEvent.parentProcessPath, 1024, &parentProcImagePath);
			}
			ExFreePoolWithTag(parentProcImagePath.Buffer, PROCESS_POOL_TAG);
		}
	}

	processEventPacket->processEvent.hParentProcessId = hParentId;
	processEventPacket->processEvent.hOriginProcessId = hParentId;
	processEventPacket->processEvent.hProcessId = hProcessId;
	processEventPacket->processEvent.hThreadId = 0;
    ...
    )
{
	...
	UNICODE_STRING processImagePath;
	//UNICODE_STRING parentProcImagePath;
	UNICODE_STRING originProcImagePath;
	...
	/* Look up origin (==current) process ID */
	hOriginId = PsGetCurrentProcessId();

	/* Look up parent process ID */
	status = ZwQueryInformationProcess(hProcess, 0, &pbi, sizeof(pbi), &len);
	if (NT_SUCCESS(status)) 
	{
		hParentId = (HANDLE) pbi.InheritedFromUniqueProcessId;
	}
	else
	{
		DbgPrint("ZwQueryInformationProcess Failed: %08x\n", status);
		return;
	}
	...
	originProcImagePath.Length = 0;
	originProcImagePath.MaximumLength = 0;
	...
	status = GetProcessImageName(hOriginId, &originProcImagePath);
	if (status == STATUS_BUFFER_OVERFLOW)
	{
		originProcImagePath.Buffer = ExAllocatePoolWithTag(NonPagedPool, originProcImagePath.MaximumLength, PROCESS_POOL_TAG);
		if (originProcImagePath.Buffer != NULL)
		{
			status = GetProcessImageName(hOriginId, &originProcImagePath);
			if (NT_SUCCESS(status))
			{
				//DbgPrint("CaptureProcessMonitor: %i %i=>%i:%wZ\n", bCreate, hParentId, hProcessId, &processImagePath);
				RtlStringCbCopyUnicodeString(processEventPacket->processEvent.originProcessPath, 1024, &originProcImagePath);
			}
			ExFreePoolWithTag(originProcImagePath.Buffer, PROCESS_POOL_TAG);
		}
	}
	...
	//DbgPrint("CaptureProcessMonitor (Thread): %i Process %i->%i(%i) Thread %i\n", bCreate, hOriginId, hProcessId, hParentId, hThreadId);

	RtlCopyMemory(&processEventPacket->processEvent.time, &timeFields, sizeof(TIME_FIELDS));
	processEventPacket->processEvent.hParentProcessId = hParentId;
	processEventPacket->processEvent.hOriginProcessId = hOriginId;
	processEventPacket->processEvent.hProcessId = hProcessId;
	processEventPacket->processEvent.hThreadId = hThreadId;
	processEventPacket->processEvent.bCreated = bCreate;
...
void UnloadDriver(IN PDRIVER_OBJECT DriverObject)
{
	...
	if(pProcessManager->bReady)
	{
		...
		ntStatus = PsRemoveCreateThreadNotifyRoutine(ThreadCallback);

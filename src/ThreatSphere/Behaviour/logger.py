...
handler = logging.handlers.RotatingFileHandler(LOG_FILE, maxBytes = 1024*1024*200, backupCount = 5) # ...handler    
fmt = '%(asctime)s - %(filename)s:%(lineno)s - %(name)s -  %(threadName)s - %(message)s'  

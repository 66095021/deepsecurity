...
//	DbgPrint("InsertHashMap:map=%08x k=%08x v=%08x begin...\n",hash_map, key, value);
...
//	DbgPrint("InsertHashMap:map=%08x k=%08x v=%08x done.\n",hash_map, key, value);
...
//	DbgPrint("FindHashMap\t\t\tmap = %08x key = %08x hash=%u \n",hash_map, key,hash);

	if( hash_map->map[hash] == NULL )
	{
//		DbgPrint("FindHashMap NULL \t\t\tmap = %08x key = %08x hash=%u \n",hash_map, key,hash);
		return NULL;
	}
...
//	DbgPrint("FindHashMap no pair \t\t\tmap = %08x key = %08x hash=%u \n",hash_map, key,hash);
	return NULL;
}

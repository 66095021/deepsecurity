...
DWORD ProcessHandler::executeProcess() 
{
     ...
     */
	int isize = m_parameters.size() + m_fullPathToExe.size() + 2;
     wchar_t * pwszParam = new wchar_t[isize];
     if (pwszParam == 0)
     {
          return 1;
     }
	 swprintf_s(pwszParam,isize, L"%s %s",m_fullPathToExe.c_str(), m_parameters.c_str());
     ...
     if (!CreateProcessW(NULL, pwszParam, 0, 0, true,

...
NetworkMonitor::processUdpEvents(KernelEventList* udp_event_list)
{
	if( udp_event_list->head )
	{
		...
		while(data_offset)
		{
			...
			if( data->key == 0)
			{
				...
				if(connection_state == UDP_STATE_SEND ||
					...
					connection_state == UDP_STATE_CONNECT)
				{
			...
			else if(data->key == 1)
			{
				...
				source_address = StringHelper::multiByteStringToWideString(address, strlen(address)+1);
		...
		{
			signal_onConnectionEvent(udp_type, Time::getCurrentTime(), process_id, process_path, source_address, destination_address);
		}
...
NetworkMonitor::processTcpEvents(KernelEventList* tcp_event_list)
{
	if( tcp_event_list->head )
	{
		...
		{
			signal_onConnectionEvent(connection_type, Time::getCurrentTime(), process_id, process_path, source_address, destination_address);
		}

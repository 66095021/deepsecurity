...
Analyzer::start()
{
	...
	onNetworkEventConnection = networkMonitor->connect_onConnectionEvent(boost::bind(&Analyzer::onConnectionEvent, this, _1, _2, _3, _4, _5, _6));
...
void
Analyzer::onProcessEvent(DWORD eventType, const std::wstring& time, 
						 ...
						 DWORD processId, const std::wstring& process)
{
	...
	switch(eventType)
	{
	case EventProcess_Create:
		processType = L"created";
		break;
	case EventProcess_Terminate:
		processType = L"terminated";
		break;
	case EventThread_Inject:
		processType = L"injected";
		break;
	}
...
Analyzer::onConnectionEvent(const std::wstring& networkEventType, const std::wstring& time, const DWORD processId, 
						 const std::wstring& processPath, const std::wstring& networkSrcPath, const std::wstring& networkDstPath)
{
	std::wstring networkEvent = L"connection";
	sendSystemEvent(networkEvent, time, processId,
					processPath, networkEventType, networkSrcPath, networkDstPath);
}
